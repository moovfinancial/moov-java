...
public class CreateBankAccountAttestation {
    ...
    public static class Sync extends Base
            implements RequestOperation<CreateBankAccountAttestationRequest, CreateBankAccountAttestationResponse> {
        public Sync(SDKConfiguration sdkConfiguration, Headers _headers) {
            super(sdkConfiguration, _headers);
        }

        private HttpRequest onBuildRequest(CreateBankAccountAttestationRequest request) throws Exception {
            HttpRequest req = buildRequest(request, CreateBankAccountAttestationRequest.class, new TypeReference<CreateBankAccountAttestationRequest>() {});
        ...
        @Override
        public HttpResponse<InputStream> doRequest(CreateBankAccountAttestationRequest request) {
        ...
        @Override
        public CreateBankAccountAttestationResponse handleResponse(HttpResponse<InputStream> response) {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            CreateBankAccountAttestationResponse.Builder resBuilder =
            ...
            CreateBankAccountAttestationResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "200")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    return res.withBankAccountAttestation(Utils.unmarshal(response, new TypeReference<BankAccountAttestation>() {}));
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    throw BankAccountAttestationValidationError.from(response);
                } else {
                    throw APIException.from("Unexpected content-type received: " + contentType, response);
                }
            }
