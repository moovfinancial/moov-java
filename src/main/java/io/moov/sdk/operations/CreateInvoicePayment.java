...
public class CreateInvoicePayment {
    ...
    public static class Sync extends Base
            implements RequestOperation<CreateInvoicePaymentRequest, CreateInvoicePaymentResponse> {
        public Sync(SDKConfiguration sdkConfiguration, Headers _headers) {
            super(sdkConfiguration, _headers);
        }

        private HttpRequest onBuildRequest(CreateInvoicePaymentRequest request) throws Exception {
            HttpRequest req = buildRequest(request, CreateInvoicePaymentRequest.class, new TypeReference<CreateInvoicePaymentRequest>() {});
        ...
        @Override
        public HttpResponse<InputStream> doRequest(CreateInvoicePaymentRequest request) {
        ...
        @Override
        public CreateInvoicePaymentResponse handleResponse(HttpResponse<InputStream> response) {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            CreateInvoicePaymentResponse.Builder resBuilder =
            ...
            CreateInvoicePaymentResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "200")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    return res.withInvoicePayment(Utils.unmarshal(response, new TypeReference<InvoicePayment>() {}));
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    throw CreateInvoicePaymentError.from(response);
                } else {
                    throw APIException.from("Unexpected content-type received: " + contentType, response);
                }
            }
