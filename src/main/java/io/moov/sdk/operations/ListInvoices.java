...
public class ListInvoices {
    ...
    public static class Sync extends Base
            implements RequestOperation<ListInvoicesRequest, ListInvoicesResponse> {
        public Sync(SDKConfiguration sdkConfiguration, Headers _headers) {
            super(sdkConfiguration, _headers);
        }

        private HttpRequest onBuildRequest(ListInvoicesRequest request) throws Exception {
            HttpRequest req = buildRequest(request, ListInvoicesRequest.class);
        ...
        @Override
        public HttpResponse<InputStream> doRequest(ListInvoicesRequest request) {
        ...
        @Override
        public ListInvoicesResponse handleResponse(HttpResponse<InputStream> response) {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            ListInvoicesResponse.Builder resBuilder =
            ...
            ListInvoicesResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "200")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    return res.withInvoices(Utils.unmarshal(response, new TypeReference<List<Invoice>>() {}));
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    throw ListInvoicesValidationError.from(response);
                } else {
                    throw APIException.from("Unexpected content-type received: " + contentType, response);
                }
            }
