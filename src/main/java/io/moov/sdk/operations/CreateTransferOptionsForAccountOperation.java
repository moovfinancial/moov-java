...
public class CreateTransferOptionsForAccountOperation implements RequestOperation<CreateTransferOptionsForAccountRequest, CreateTransferOptionsForAccountResponse> {
    
    private final SDKConfiguration sdkConfiguration;

    public CreateTransferOptionsForAccountOperation(SDKConfiguration sdkConfiguration) {
        this.sdkConfiguration = sdkConfiguration;
    }
    
    @Override
    public HttpResponse<InputStream> doRequest(CreateTransferOptionsForAccountRequest request) throws Exception {
        ...
        try {
            ...
            if (Utils.statusCodeMatches(httpRes.statusCode(), "400", "401", "403", "422", "429", "4XX", "500", "504", "5XX")) {
    ...
    @Override
    public CreateTransferOptionsForAccountResponse handleResponse(HttpResponse<InputStream> response) throws Exception {
        String contentType = response
            .headers()
            .firstValue("Content-Type")
            .orElse("application/octet-stream");
        CreateTransferOptionsForAccountResponse.Builder resBuilder = 
        ...
        CreateTransferOptionsForAccountResponse res = resBuilder.build();
        
        if (Utils.statusCodeMatches(response.statusCode(), "200")) {
            res.withHeaders(response.headers().map());
            if (Utils.contentTypeMatches(contentType, "application/json")) {
                TransferOptions out = Utils.mapper().readValue(
                    response.body(),
                    new TypeReference<>() {
                    });
                res.withTransferOptions(out);
                return res;
            } else {
        ...
        if (Utils.statusCodeMatches(response.statusCode(), "422")) {
            res.withHeaders(response.headers().map());
            if (Utils.contentTypeMatches(contentType, "application/json")) {
                TransferOptionsValidationError out = Utils.mapper().readValue(
                    response.body(),
                    new TypeReference<>() {
                    });
                throw out;
