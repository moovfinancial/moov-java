...
public class UploadDisputeEvidenceFile {
    ...
    public static class Sync extends Base
            implements RequestOperation<UploadDisputeEvidenceFileRequest, UploadDisputeEvidenceFileResponse> {
        public Sync(SDKConfiguration sdkConfiguration, Headers _headers) {
            super(sdkConfiguration, _headers);
        }

        private HttpRequest onBuildRequest(UploadDisputeEvidenceFileRequest request) throws Exception {
            HttpRequest req = buildRequest(request, UploadDisputeEvidenceFileRequest.class, new TypeReference<UploadDisputeEvidenceFileRequest>() {});
        ...
        @Override
        public HttpResponse<InputStream> doRequest(UploadDisputeEvidenceFileRequest request) throws Exception {
        ...
        @Override
        public UploadDisputeEvidenceFileResponse handleResponse(HttpResponse<InputStream> response) throws Exception {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            UploadDisputeEvidenceFileResponse.Builder resBuilder =
            ...
            UploadDisputeEvidenceFileResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "201")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    EvidenceUploadResponse out = Utils.mapper().readValue(
                            response.body(),
                            new TypeReference<>() {
                            });
                    res.withEvidenceUploadResponse(out);
                    return res;
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    FileUploadValidationError out = Utils.mapper().readValue(
                            response.body(),
                            new TypeReference<>() {
                            });
                    throw out;
