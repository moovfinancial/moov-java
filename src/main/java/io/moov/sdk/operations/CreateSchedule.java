...
public class CreateSchedule {
    ...
    public static class Sync extends Base
            implements RequestOperation<CreateScheduleRequest, CreateScheduleResponse> {
        public Sync(SDKConfiguration sdkConfiguration) {
            super(sdkConfiguration);
        }

        private HttpRequest onBuildRequest(CreateScheduleRequest request) throws Exception {
            HttpRequest req = buildRequest(request, CreateScheduleRequest.class, new TypeReference<CreateScheduleRequest>() {});
        ...
        @Override
        public HttpResponse<InputStream> doRequest(CreateScheduleRequest request) throws Exception {
        ...
        @Override
        public CreateScheduleResponse handleResponse(HttpResponse<InputStream> response) throws Exception {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            CreateScheduleResponse.Builder resBuilder =
            ...
            CreateScheduleResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "200")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    ScheduleResponse out = Utils.mapper().readValue(
                            response.body(),
                            new TypeReference<>() {
                            });
                    res.withScheduleResponse(out);
                    return res;
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    ScheduleValidationError out = Utils.mapper().readValue(
                            response.body(),
                            new TypeReference<>() {
                            });
                    throw out;
