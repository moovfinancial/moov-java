...
public class LinkAccountTerminalApplication {
    ...
    public static class Sync extends Base
            implements RequestOperation<LinkAccountTerminalApplicationRequest, LinkAccountTerminalApplicationResponse> {
        public Sync(SDKConfiguration sdkConfiguration, Headers _headers) {
            super(sdkConfiguration, _headers);
        }

        private HttpRequest onBuildRequest(LinkAccountTerminalApplicationRequest request) throws Exception {
            HttpRequest req = buildRequest(request, LinkAccountTerminalApplicationRequest.class, new TypeReference<LinkAccountTerminalApplicationRequest>() {});
        ...
        @Override
        public HttpResponse<InputStream> doRequest(LinkAccountTerminalApplicationRequest request) {
        ...
        @Override
        public LinkAccountTerminalApplicationResponse handleResponse(HttpResponse<InputStream> response) {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            LinkAccountTerminalApplicationResponse.Builder resBuilder =
            ...
            LinkAccountTerminalApplicationResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "200")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    return res.withAccountTerminalApplication(Utils.unmarshal(response, new TypeReference<AccountTerminalApplication>() {}));
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    throw AccountTerminalApplicationError.from(response);
                } else {
                    throw APIException.from("Unexpected content-type received: " + contentType, response);
                }
            }
