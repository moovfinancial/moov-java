...
public class LinkGooglePayToken {
    ...
    public static class Sync extends Base
            implements RequestOperation<LinkGooglePayTokenRequest, LinkGooglePayTokenResponse> {
        public Sync(SDKConfiguration sdkConfiguration, Headers _headers) {
            super(sdkConfiguration, _headers);
        }

        private HttpRequest onBuildRequest(LinkGooglePayTokenRequest request) throws Exception {
            HttpRequest req = buildRequest(request, LinkGooglePayTokenRequest.class, new TypeReference<LinkGooglePayTokenRequest>() {});
        ...
        @Override
        public HttpResponse<InputStream> doRequest(LinkGooglePayTokenRequest request) {
        ...
        @Override
        public LinkGooglePayTokenResponse handleResponse(HttpResponse<InputStream> response) {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            LinkGooglePayTokenResponse.Builder resBuilder =
            ...
            LinkGooglePayTokenResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "200")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    return res.withLinkedGooglePayPaymentMethods(Utils.unmarshal(response, new TypeReference<List<LinkedGooglePayPaymentMethod>>() {}));
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    throw LinkGooglePayError.from(response);
                } else {
                    throw APIException.from("Unexpected content-type received: " + contentType, response);
                }
            }
