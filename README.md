...
Gradle:
```groovy
implementation 'io.moov:openapi:0.0.3'
```
...
* [createAuthToken](docs/sdks/authentication/README.md#createauthtoken) - Use the `client_id` and `client_secret` to generate an access token.
...
By default, an API error will throw a `models/errors/APIException` exception. When custom error responses are specified for an operation, the SDK may also throw their associated exception. You can refer to respective *Errors* tables in SDK docs for more details on possible exception types for each operation. For example, the `createAccount` method throws the following exceptions:
