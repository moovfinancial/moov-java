...
public class LinkApplePayToken {
    ...
    public static class Sync extends Base
            implements RequestOperation<LinkApplePayTokenRequest, LinkApplePayTokenResponse> {
        public Sync(SDKConfiguration sdkConfiguration) {
            super(sdkConfiguration);
        }

        private HttpRequest onBuildRequest(LinkApplePayTokenRequest request) throws Exception {
            HttpRequest req = buildRequest(request, LinkApplePayTokenRequest.class, new TypeReference<LinkApplePayTokenRequest>() {});
        ...
        @Override
        public HttpResponse<InputStream> doRequest(LinkApplePayTokenRequest request) throws Exception {
        ...
        @Override
        public LinkApplePayTokenResponse handleResponse(HttpResponse<InputStream> response) throws Exception {
            String contentType = response
                    .headers()
                    .firstValue("Content-Type")
                    .orElse("application/octet-stream");
            LinkApplePayTokenResponse.Builder resBuilder =
            ...
            LinkApplePayTokenResponse res = resBuilder.build();
            
            if (Utils.statusCodeMatches(response.statusCode(), "200")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    LinkedApplePayPaymentMethod out = Utils.mapper().readValue(
                            response.body(),
                            new TypeReference<>() {
                            });
                    res.withLinkedApplePayPaymentMethod(out);
                    return res;
                } else {
            ...
            if (Utils.statusCodeMatches(response.statusCode(), "422")) {
                res.withHeaders(response.headers().map());
                if (Utils.contentTypeMatches(contentType, "application/json")) {
                    LinkApplePayError out = Utils.mapper().readValue(
                            response.body(),
                            new TypeReference<>() {
                            });
                    throw out;
