...
public class CreateFeePlanAgreementsOperation implements RequestOperation<CreateFeePlanAgreementsRequest, CreateFeePlanAgreementsResponse> {
    ...
    public CreateFeePlanAgreementsOperation(SDKConfiguration sdkConfiguration) {
    ...
    public HttpRequest buildRequest(CreateFeePlanAgreementsRequest request) throws Exception {
    ...
    @Override
    public HttpResponse<InputStream> doRequest(CreateFeePlanAgreementsRequest request) throws Exception {
    ...
    @Override
    public CreateFeePlanAgreementsResponse handleResponse(HttpResponse<InputStream> response) throws Exception {
        String contentType = response
            .headers()
            .firstValue("Content-Type")
            .orElse("application/octet-stream");
        CreateFeePlanAgreementsResponse.Builder resBuilder = 
        ...
        CreateFeePlanAgreementsResponse res = resBuilder.build();
        
        if (Utils.statusCodeMatches(response.statusCode(), "201")) {
            res.withHeaders(response.headers().map());
            if (Utils.contentTypeMatches(contentType, "application/json")) {
                FeePlanAgreement out = Utils.mapper().readValue(
                    response.body(),
                    new TypeReference<>() {
                    });
                res.withFeePlanAgreement(out);
                return res;
            } else {
        ...
        if (Utils.statusCodeMatches(response.statusCode(), "422")) {
            res.withHeaders(response.headers().map());
            if (Utils.contentTypeMatches(contentType, "application/json")) {
                FeePlanAgreementError out = Utils.mapper().readValue(
                    response.body(),
                    new TypeReference<>() {
                    });
                throw out;
